...
{
    CRPT->TDES_CTL = (u32Channel << CRPT_TDES_CTL_CHANNEL_Pos) |
                       (u32EncDec << CRPT_TDES_CTL_ENCRPT_Pos) |
                       (u32OpMode << CRPT_TDES_CTL_OPMODE_Pos) |
                       (u32SwapType << CRPT_TDES_CTL_BLKSWAP_Pos);
    g_TDES_CTL[u32Channel] = CRPT->TDES_CTL;
}
...
void TDES_Start(int32_t u32Channel, uint32_t u32DMAMode)
{
    ...
    CRPT->TDES_CTL |= CRPT_TDES_CTL_START_Msk | (u32DMAMode << CRPT_TDES_CTL_DMALAST_Pos);
}
