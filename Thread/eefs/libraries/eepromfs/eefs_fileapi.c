...
int32 EEFS_LibInitFS(EEFS_InodeTable_t *InodeTable, uint32 BaseAddress)
{
    ...
    EEFS_LIB_LOCK;
    if (InodeTable != NULL) {
				   //首先试图读取 EEPROM中的  BaseAddress（一般是开始的地方）开始FileAllocationTable数据
        ...
        if ((FileAllocationTableHeader.Magic == EEFS_FILESYS_MAGIC) &&
            ...
            (FileAllocationTableHeader.NumberOfFiles <= EEFS_MAX_FILES)) {
			   //如果预先存入信息则读取加载到当前索引表中
            /* Initialize the Inode Table */
            memset(InodeTable, 0, sizeof(EEFS_InodeTable_t));//清空索引表
            InodeTable->BaseAddress = BaseAddress;//基地址
            InodeTable->FreeMemoryPointer = (void *)(InodeTable->BaseAddress + FileAllocationTableHeader.FreeMemoryOffset);	//EEROM开始地址+FreeMemory偏移
...
int32 EEFS_LibCreatFile(EEFS_InodeTable_t *InodeTable, char *Filename, uint32 Attributes)
{
    ...
    EEFS_FileHeader_t                   FileHeader;//文件头
    int32                               InodeIndex;//索引号
    int32                               FileDescriptor;//文件目标指针
    int32                               ReturnCode;//返回状态码
            ...
            if (EEFS_LibHasOpenCreat(InodeTable) == FALSE) {
                ...
                if (InodeTable->FreeMemorySize > sizeof(EEFS_FileHeader_t)) {
                    ...
                    if ((Attributes == EEFS_ATTRIBUTE_NONE) || (Attributes == EEFS_ATTRIBUTE_READONLY)) {
									 //属性为EEFS_ATTRIBUTE_NONE或者EEFS_ATTRIBUTE_READONLY可以创建
                        if ((FileDescriptor = EEFS_LibGetFileDescriptor()) != EEFS_NO_FREE_FILE_DESCRIPTOR) {
									  //在文件描述索引表中分配一个空的文件描述索引
                            ...
                            InodeIndex = InodeTable->NumberOfFiles;//文件数量
                            InodeTable->NumberOfFiles++; //文件数量增加一个表示新建了一个文件
                            InodeTable->File[InodeIndex].FileHeaderPointer = InodeTable->FreeMemoryPointer;	//文件头指针指向空闲内存储指针
                            InodeTable->File[InodeIndex].MaxFileSize = (InodeTable->FreeMemorySize - sizeof(EEFS_FileHeader_t));//最大文件尺寸=空闲空间-去文件头部尺寸
                            ...
                            /* Initialize the File Descriptor */								/*add read function*/
...
{
    uint8      *BeginningOfFilePointer;
    uint8       *EndOfFilePointer;
    int32       ReturnCode;
