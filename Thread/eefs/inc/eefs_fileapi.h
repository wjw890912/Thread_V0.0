...
typedef struct
{
    ...
    time_t                              ModificationDate;  //修改日期
    time_t                              CreationDate;	//创建日期
    char                                Filename[EEFS_MAX_FILENAME_SIZE]; //文件名
} EEFS_FileHeader_t;

typedef struct
{
    uint8                               *FileHeaderPointer;	//文件头指针
    uint32                              MaxFileSize;   //文件尺寸
} EEFS_InodeTableEntry_t;

typedef struct
{
    uint32                              BaseAddress; //基地址
    uint8                               *FreeMemoryPointer;	//孔空间指针
    uint32                              FreeMemorySize;	 //空空间
    uint32                              NumberOfFiles; //文件数量
    EEFS_InodeTableEntry_t              File[EEFS_MAX_FILES]; //文件
} EEFS_InodeTable_t;
...
typedef struct
{
    uint32                              InUse;
    uint32                              Mode;
    uint8                                *FileHeaderPointer;
    uint8                               *FileDataPointer;
    uint32                              ByteOffset;
    uint32                              FileSize;
    uint32                              MaxFileSize;
