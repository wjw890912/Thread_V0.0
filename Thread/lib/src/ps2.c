 ...
 *
 *  @param    u32Mask    The specified interrupt of PS2 module:
 *                               - PS2_CTL_TXINTEN_Msk: PS2 Tx interrupt
 *                               - PS2_CTL_RXINTEN_Msk: PS2 Rx interrupt
 *
 *  @return   None
 */
...
int32_t PS2_Write(uint32_t *pu32Buf, uint32_t u32ByteCount)
{
    ...
    do
    {
        ...
        PS2->TXDAT1 = pu32Buf[i+4];
        PS2->TXDAT2 = pu32Buf[i+8];
        PS2->TXDAT3 = pu32Buf[i+12];
